    ...
    <li><a class="dropdown-item" href="ingreso.php">Salir</a></li>
        ...
        <form class="row g-3" action="" method="post">
            <div class="col-auto d-flex">
                <input type="text" class="form-control form-control-sm me-2" name="busqueda" placeholder="Buscar productos...">
                <button type="submit" class="btn btn-light btn-sm" name="enviar"><span><i class="fa-solid fa-magnifying-glass"></i></span></button>
                <button class="btn btn-dark" onclick="window.location.href='catalogo.php';">Restablecer</button>  
            </div>
        </form>    
    </nav>
    <div >
        <!-- Barra de búsqueda -->
            <?php
                if (isset($_POST['enviar'])) {
                    ...
                    // Consulta para obtener productos por nombre o descripción
                    $consulta_busqueda = $miPDO->prepare("SELECT * FROM producto WHERE nombre LIKE :busqueda");
                    $consulta_busqueda->bindValue(':busqueda', "%$busqueda%", PDO::PARAM_STR);
                    $consulta_busqueda->execute();

                    if ($consulta_busqueda === false) {
                        // Manejar errores de consulta
                        echo "Error en la consulta SQL.";
                    } else {
                        // Mostrar resultados de la consulta
                        while ($row = $consulta_busqueda->fetch(PDO::FETCH_ASSOC)) {   
                            echo '<a class="btn btn-light d-flex align-items-center justify-content-between mb-3 text-decoration-none text-dark" href="categoria_camisas.php?clasificacion=' . $row['clasificacion'] . '"><h3 class="mb-0 fs-6">' . $row['nombre'] . '</h3>' . '<i class="fa-solid fa-caret-right"></i>' . '</a>';
                        }
